...
def merge_configs(base: dict, override: dict) -> dict:
    ...
    merged["statements"] = list(dict.fromkeys(base.get("statements", []) + override.get("statements", [])))
    ...
    for qt_name, qt_data in override.get("question_types", {}).items():
        if qt_name in merged_qt:
            ...
            merged_qt[qt_name]["options"] = {**merged_qt[qt_name].get("options", {}), **qt_data.get("options", {})}
        else:
...
def apply_template_to_course(course, templates_dir=TEMPLATES_DIR):
    ...
    for qt_name, qt_data in template_data.get("question_types", {}).items():
        ...
        qt_statements = list(dict.fromkeys(top_level_statements + qt_data.get("statements", [])))
        ...
        for opt_name, opt_data in qt_data.get("options", {}).items():
            ...
            options.append(QuestionTypeOption(name=opt_name, statements=option_statements))

        # Create the QuestionType object
        question_types.append(
            QuestionType(
                name=qt_name,
                statements=qt_statements,
                options=options
            )
        )
