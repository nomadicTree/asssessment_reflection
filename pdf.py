from reportlab.platypus import SimpleDocTemplate, Paragraph, Image, Spacer, Table, TableStyle, ListFlowable, ListItem
...
def create_summary_pdf(ar, output_path):
    ...
    elements.append(Paragraph(f"<b>Name:</b> {ar.student_name}", styles["Normal"]))
    elements.append(Paragraph(f"<b>Assessment:</b> {ar.assessment_name}", styles["Normal"]))
    ...
    for r in ar.reflections:
        ...
        elements.append(Paragraph(f"<b>Question type:</b> {r.question_type.name}"))
        elements.append(Spacer(1, 4))
        ...
        statement_list_items = [ListItem(Paragraph(point, styles["Normal"])) for point in r.selected_statements]
        ...
        for option in r.selected_options:
            ...
            statement_list_items = [ListItem(Paragraph(point, styles["Normal"])) for point in r.selected_options[option]]
